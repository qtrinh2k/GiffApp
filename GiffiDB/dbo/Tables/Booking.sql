CREATE TABLE [dbo].[Booking] (
    [Id]            INT            NOT NULL,
    [CreatedBy]     NCHAR (50)     NOT NULL,
    [CreatedTime]   DATETIME       NOT NULL DEFAULT GETDATE(),
    [ModifiedTime]  DATETIME       NOT NULL DEFAULT GETDATE(),
    [BillToId]     INT            NOT NULL,
    [ShipperId]     INT            NOT NULL,
    [CarrierId]     INT            NOT NULL,
    [Vessel]        NVARCHAR (50)  NOT NULL,
    [VSL]           NCHAR (10)     NOT NULL,
    [OriginId]      INT            NOT NULL,
    [LoadId]        INT			   NOT NULL,
    [DischargeId]   INT            NOT NULL,
    [DestinationId] INT            NOT NULL,
    [Commodity]     NVARCHAR (200) NULL,
    [Equiment]      NVARCHAR (100)  NOT NULL,
    [Temp]          NCHAR (10)     NOT NULL,
    [Vents]         NCHAR (10)     NOT NULL,
    [Notes]         ntext NULL,
    [Status] NCHAR(10) NULL, 
    PRIMARY KEY CLUSTERED ([Id] ASC), 
	CONSTRAINT [FK_Booking_BillTo] FOREIGN KEY ([BillToId]) REFERENCES [Company]([Id]),
	CONSTRAINT [FK_Booking_Shipper] FOREIGN KEY ([ShipperId]) REFERENCES [Company]([Id]),
	CONSTRAINT [FK_Booking_Carrier] FOREIGN KEY ([CarrierId]) REFERENCES [Carrier]([Id]),
    CONSTRAINT [FK_Booking_Origin] FOREIGN KEY ([OriginId]) REFERENCES [Location]([Id]),
	CONSTRAINT [FK_Booking_Load] FOREIGN KEY ([LoadId]) REFERENCES [Location]([Id]),
	CONSTRAINT [FK_Booking_Discharge] FOREIGN KEY ([DischargeId]) REFERENCES [Location]([Id]),
	CONSTRAINT [FK_Booking_Destination] FOREIGN KEY ([DestinationId]) REFERENCES [Location]([Id])
);

